...
## Usuarios
def guardar_usuario(datos, columnas, df=None):
...
if __name__ == "__main__":
    ...
    df = guardar_usuario(datos, columnas)

    # Ejemplo de añadir datos a un dataframe existente
    nuevos_datos = [["Luis", 22, "Valencia"]]
    df = guardar_usuario(nuevos_datos, columnas, df)

    print(df)
    # ---
    # Ejemplo de guardar en un archivo de texto
    guardar_en_archivo(df, 'ejemplo.txt')

    # Ejemplo de leer desde archivo
    df_leido= leer_desde_archivo('ejemplo.txt')
